...
class Model extends \Scern\Lira\Model
{
    ...
    {
        try{
            $query = $this->database->prepare("SELECT * FROM {$this->table} AS c,{$this->table_content} AS cc WHERE c.id=cc.id_category AND cc.language = :language ORDER BY created DESC");
            $query->execute(['ru']);
